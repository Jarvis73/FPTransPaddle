...
def config():
    ...
    proc = "cv2"
    sigma = 5
    save_bytes = False       # save weights in uint8 format for saving space (especially for COCO)
    weights_save_dir = DATA_DIR[dataset] / "weights"
    dry_run = False


def boundary2weight(target, cls, kernel, sigma=5):
    mask = paddle.zeros(target.shape, dtype=paddle.float32).cuda()
    mask[target == cls] = 1
    mask = mask.unsqueeze(dim=0)      # [1, H, W]
    mask = mask.unsqueeze(dim=0)      # [1, 1, H, W]

    # Extract mask boundary (inner and outer)
    dilated = paddle.clip(F.conv2d(mask, kernel, padding=1), 0, 1) - mask
    erosion = mask - paddle.clip(F.conv2d(mask, kernel, padding=1) - 8, 0, 1)
...
def precompute_loss_weights(_config):
    ...
    kernel = paddle.ones((1, 1, 3, 3), dtype=paddle.float32).cuda()
...
def print_ckpt(ckpt):
    ...
    """
    state = paddle.load(ckpt, map_location='cpu')
