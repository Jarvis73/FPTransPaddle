...
def init_device(net):
    ...
    # 2. create data parallel model
    net_DP = paddle.DataParallel(net)
    return net_DP
...
class BaseEvaluator(object):
    ...
    def __init__(self, opt, logger, device, model, model_T, mode, need_grad=False):
        ...
        self.num_devices = paddle.device.cuda.device_count()

        self.model = model
        if not isinstance(self.model, paddle.DataParallel):
            self.model_DP = init_device(self.model)
        else:
    ...
    def start_eval_loop(self, data_loader, num_classes, epoch=None):
        ...
        for i, batch in enumerate(gen, start=1):
            ...
            # save prediction
            if save_dir:
                qry_mask = batch['qry_msk'].numpy()[:, 0]
                qry_pred = qry_pred[:, 0]
                classes = batch['cls'].numpy().tolist()
...
class BaseTrainer(object):
    ...
    def print_lr(self):
        # TODO: Can Paddle set lr for each param_group?
        self.logger.info(f"Learning rate for {self.opt.optim.upper()} is {self.optimizer.get_lr():.6g}")
    ...
    def step_lr(self):
        ...
        if self.opt.lrp in ["cosine", "poly"]:   # forward per step
            self.scheduler.step()
    ...
    def snapshot(self, epoch, miou, biou, verbose=0):
        ...
        save_path = self.log_dir / "ckpt.pdparams"
        state = {
            'epoch': epoch,
            'miou': miou,
            'biou': biou,
        ...
        if best:
            shutil.copyfile(save_path, self.log_dir / "bestckpt.pdparams")
        ...
        if on_cloud:
            state.update({
                ...
                'paddle_random_state': paddle.get_cuda_rng_state(),
                ...
                # TODO: How to get state dicts of the optimizer and scheduler?
                # 'optimizer_state': self.optimizer.state_dict(),
                # 'scheduler_state': self.scheduler.state_dict(),
            })

            try:
                on_cloud_save_path = self.cloud_save_dir / "ckpt.pdparams"
                if on_cloud_save_path.exists():
                    os.remove(on_cloud_save_path)
                paddle.save(state, on_cloud_save_path)
            except Exception as e:
                print(e)
            
            if best:
                try:
                    on_cloud_save_path_best = self.cloud_save_dir / "bestckpt.pdparams"
