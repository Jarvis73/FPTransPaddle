...
paddlevision and its usage is the same as paddlevision, except for all the
...
class ToTensor(object):
    """
    PaddlePaddle does not support GPU Tensor in multiprocess DataLoader. 
    Make sure that to_tensor operation has `place='cpu'`.
    """
    ...
    def __call__(self, img, mask, proc='pil', **kwargs):
        ...
        mask = paddle.to_tensor(np.array(mask, np.uint8), dtype=paddle.uint8, place='cpu')
        if self.mask_dtype == 'long':
            mask = paddle.cast(mask, paddle.int64)
        elif self.mask_dtype == 'float':
            mask = paddle.cast(mask, paddle.float32)

        if 'weights' in kwargs:
            kwargs['weights'] = paddle.to_tensor(kwargs['weights'], dtype=paddle.float32, place='cpu')
